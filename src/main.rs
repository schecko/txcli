...
impl From<InputTx> for Tx
{
    fn from(input: InputTx) -> Self {
        Tx{ tx_type: input.0, cid: ClientId( input.1 ), tid: TxId( input.2 ), amount: input.3 }
    }
}
...
impl Tx
{
    ...
    {
        Tx{ tx_type: ty, cid: ClientId( cid ), tid: TxId( tid ), amount }
    }
}
...
{
    let mut client_entry = app_state.clients.entry(tx.cid).or_insert(ClientState::default());
    ...
    match &tx.tx_type {
        ...
        TxType::Dispute => {
            if let Some(previous_tx) = client_entry.history.remove(&tx.tid) {
                ...
                eprintln!( "Detected dispute referencing unknown previous transaction tid[{}]. Ignoring.", tx.tid.0 );
            }
        },
        TxType::Resolve => {
            if let Some(previous_tx) = client_entry.disputed.remove(&tx.tid) {
                ...
                eprintln!( "Detected resolve referencing unknown disputed transaction tid[{}]. Ignoring.", tx.tid.0 );
            }
        },
        TxType::ChargeBack => {
            if let Some(previous_tx) = client_entry.disputed.remove(&tx.tid) {
...
mod tests
{
    ...
    {
        let mut app_state = AppState::default();
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 1, 1, Currency::from_num( 1.0 ) ) );
        assert_eq!( app_state.clients.len(), 1 );
        assert_eq!( app_state.clients.entry( ClientId( 1 ) ).or_default().available, Currency::from_num( 1.0 ) );
    }
    ...
    fn basic_deposit_multi_user()
    {
        let mut app_state = AppState::default();
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 1, 1, Currency::from_num( 1.0 ) ) );
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 2, 1, Currency::from_num( 1.0 ) ) );
        assert_eq!( app_state.clients.len(), 2 );
        assert_eq!( app_state.clients.entry( ClientId( 1 ) ).or_default().available, Currency::from_num( 1.0 ) );
        assert_eq!( app_state.clients.entry( ClientId( 2 ) ).or_default().available, Currency::from_num( 1.0 ) );
    }
    ...
    fn basic_withdrawal()
    {
        let mut app_state = AppState::default();
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 1, 1, Currency::from_num( 1.0 ) ) );
        execute_transaction( &mut app_state, Tx::new( TxType::Withdrawal, 1, 2, Currency::from_num( 0.5 ) ) );
        assert_eq!( app_state.clients.len(), 1 );
        assert_eq!( app_state.clients.entry( ClientId( 1 ) ).or_default().available, Currency::from_num( 0.5 ) );
    }
    ...
    fn dispute_txid_doesnt_exist()
    {
        let mut app_state = AppState::default();
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 1, 1, Currency::from_num( 1.0 ) ) );
        execute_transaction( &mut app_state, Tx::new( TxType::Dispute, 1, 0, Currency::default() ) );
        assert_eq!( app_state.clients.len(), 1 );
        let client_state = app_state.clients.entry( ClientId( 1 ) ).or_default();
    ...
    fn resolve_txid_doesnt_exist()
    {
        ...
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 1, 1, Currency::from_num( 1.0 ) ) );
        execute_transaction( &mut app_state, Tx::new( TxType::Dispute, 1, 1, Currency::default() ) );
        execute_transaction( &mut app_state, Tx::new( TxType::Resolve, 1, 0, Currency::default() ) );
        assert_eq!( app_state.clients.len(), 1 );
        let client_state = app_state.clients.entry( ClientId( 1 ) ).or_default();
        assert_eq!( client_state.available, Currency::from_num( 0.0 ) );
        assert_eq!( client_state.held, Currency::from_num( 1.0 ) );
    ...
    fn chargeback_txid_doesnt_exist()
    {
        ...
        execute_transaction( &mut app_state, Tx::new( TxType::Deposit, 1, 1, Currency::from_num( 1.0 ) ) );
        execute_transaction( &mut app_state, Tx::new( TxType::Dispute, 1, 1, Currency::default() ) );
        execute_transaction( &mut app_state, Tx::new( TxType::ChargeBack, 1, 0, Currency::default() ) );
        assert_eq!( app_state.clients.len(), 1 );
        let client_state = app_state.clients.entry( ClientId( 1 ) ).or_default();
        assert_eq!( client_state.available, Currency::from_num( 0.0 ) );
        assert_eq!( client_state.held, Currency::from_num( 1.0 ) );
